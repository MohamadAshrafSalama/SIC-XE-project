...
Maps mnemonic names to their opcode values and instruction formats.
"""

# Format: {mnemonic: (opcode_hex, format)}
OPCODE_TABLE = {
    'ADD':    (0x18, 3),
    'ADDF':   (0x58, 3),
    'ADDR':   (0x90, 2),
...
# Assembler directives (not actual machine instructions)
...
def get_opcode(mnemonic):
    """Look up opcode and format for a given mnemonic."""
    mnemonic = mnemonic.lstrip('+')
...
def get_format(mnemonic):
    ...
    if clean in OPCODE_TABLE:
        ...
        if mnemonic.startswith('+') and fmt == 3:
            return 4
        return fmt
    return None
